...
def run_game(screen):
    ...
    while running: 
        ...
        screen.fill((0,0,0))  # Xóa khung hình cũ mỗi loop
        ...
        for x in object_group:
            if not isinstance(x,(ga.Mario,ga.bg)):
                ...
                x.update()
                screen.blit(x.image, (x.rect.x+plus,x.rect.y) if plus < 0 else (x.rect.x,x.rect.y))
...
def popup_highest():
    ...
    while popup_running:
        ...
        if max_scroll > 0:
            ...
            # pg.draw.rect(popup_screen, (0,0,0), scrollbar_rect)  # màu xám
